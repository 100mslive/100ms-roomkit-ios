...
struct HMSPeerOptionsButtonView<Content: View>: View {
    ...
    @ObservedObject var audioTrackModel: HMSTrackModel
    @ObservedObject var videoTrackModel: HMSTrackModel
    
    internal init(peerModel: HMSPeerModel, dismiss: (() -> Void)? = nil, @ViewBuilder label: @escaping (() -> Content)) {
        self.peerModel = peerModel
#if !Preview
        self.audioTrackModel = peerModel.regularAudioTrackModel ?? HMSTrackModel.empty
        self.videoTrackModel = peerModel.regularVideoTrackModel  ?? HMSTrackModel.empty
#else
        self.audioTrackModel = HMSTrackModel()
        self.videoTrackModel = HMSTrackModel()
#endif
        self.label = label
            ...
            label()
                ...
                .onChange(of: menuAction) { value in
                    ...
                    case .videoMuteToggle:
                        Task {
                            try await videoTrackModel.toggleMute()
                        }
                    case .audioMuteToggle:
                        Task {
                            try await audioTrackModel.toggleMute()
                        }
                    case .removeParticipant:
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ForEach(context.actions, id: \.self) { action in
                    ...
                    case .audioMuteToggle(let isMute):
                        HMSPeerAudioMuteOptionView(isMute: isMute)
                            .background(.white.opacity(0.0001))
                            .onTapGesture {
                                context.action = .none
                    ...
                    case .videoMuteToggle(let isMute):
                        HMSPeerVideoMuteOptionView(isMute: isMute)
                            .background(.white.opacity(0.0001))
                            .onTapGesture {
                                context.action = .none
                                context.action = action
