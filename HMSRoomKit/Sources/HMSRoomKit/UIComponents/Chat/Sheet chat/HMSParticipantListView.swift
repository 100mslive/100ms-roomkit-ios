...
extension HMSPeerModel {
    func popoverContext(roomModel: HMSRoomModel, roomInfoModel: HMSRoomInfoModel, isPresented: Binding<Bool>, menuAction: Binding<HMSPeerOptionsViewContext.Action>) -> HMSPeerOptionsViewContext? {
        ...
        if !isLocal, let audioTrackModel = audioTrackModel {
#if !Preview
            volume = Binding(get: { (audioTrackModel.track as? HMSRemoteAudioTrack)?.volume() ?? 0.0 }, set: { (audioTrackModel.track as? HMSRemoteAudioTrack)?.setVolume($0) })
#endif
            volume = .constant(0)
        }
        ...
        return HMSPeerOptionsViewContext(isPresented: isPresented, action: menuAction, volume: volume, name: name + (isLocal ? " (You)" : ""), role: role?.name.capitalized ?? "", actions: actions)
    }
}
